...
사용자 입력
   ↓
prepare_input
   ├─ structured_risk_agent   (구조화 위험 판정)
   ├─ knowledge_agent         (소형 KB 검색 + RRF Top 3)   ← 병렬
   └─ policy_agent            (위험 수준별 허용 행동)
   ↓ conservative_fusion
ollama_generate  →  policy_guardrail  →  safety_verifier  →  finalize
```
...
경량 RAG와 RRF 리랭킹, 로컬/클라우드 모델 라우터, 정책 보호 응답과 Safety 검증,
멀티턴 세션, 회귀 평가셋.
